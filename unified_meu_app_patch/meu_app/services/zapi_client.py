...
class ZapiClient:
    ...
    def __init__(self, instance_id: Optional[str] = None, token: Optional[str] = None, base_url: Optional[str] = None):
        ...
        if not self.instance_id or not self.token:
            raise ValueError("ZAPI_INSTANCE_ID e ZAPI_TOKEN são obrigatórios. Defina via env.")

    def _url(self, endpoint: str) -> str:
        return f"{self.base_url}/instances/{self.instance_id}/token/{self.token}/{endpoint.lstrip('/')}"

    def send_text(self, phone: str, text: str, message_id_to_reply: Optional[str] = None) -> Dict[str, Any]:
        url = self._url("send-text")
        payload = {"phone": phone, "message": text}
        if message_id_to_reply:
            payload["messageId"] = message_id_to_reply  # reply threading
